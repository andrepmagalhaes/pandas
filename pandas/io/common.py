...
if compat.PY3:
    ...
    from urllib import pathname2url, urlencode  # noqa
...
def get_filepath_or_buffer(
    ...
    if isinstance(
        filepath_or_buffer, (compat.string_types, compat.binary_type, mmap.mmap)
    ):
...
if compat.PY3 and not compat.FORCE_PY2:  # pragma: no cover
    ...
    class UnicodeWriter(object):
        ...
        def writerow(self, row):
            def _check_as_is(x):
                return (
                    self.quoting == csv.QUOTE_NONNUMERIC and is_number(x)
                ) or isinstance(x, str)

            row = [
                x if _check_as_is(x) else pprint_thing(x).encode("utf-8") for x in row
            ]
